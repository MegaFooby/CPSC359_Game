...
import pygame, random, time, os, Accel
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if y >= 20 and (self.rect.x + self.rect.width) > 100:
            if y < 70:
                self.pos_x = -int(y/7)
            else:
                self.pos_x = -10
        if y <= -20 and self.rect.x < self.area.width - 100:
            if y > -70:
                self.pos_x = -int(y/7)
            else:
                self.pos_x = 10
        if x >= 20 and self.rect.y + self.rect.height > 100:
            if x < 70:
                self.pos_y = -int(x/7)
            else:
                self.pos_y = -10
        if x <= -20 and self.rect.y < self.area.height - 100:
            if x > -70:
                self.pos_y = -int(x/10)
            else:
                self.pos_y = 8
